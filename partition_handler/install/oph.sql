CREATE OR REPLACE PACKAGE oph AUTHID DEFINER IS
  /*****************************************************************************
  * Partition Handler
  *
  * Author:    Thomas Krahn
  ...
  *            oph_tools >= 0.9.10
  *****************************************************************************/
  gc_module_version CONSTANT module_admin.t_module_version := '0.9.35';
  ...
  ******************************************************************************/
  ...
  CURSOR cur_obj_defs IS
    SELECT * FROM oph_objects o WHERE o.enabled = oph_constants.c_enabled ORDER BY o.object_type DESC, o.order# ASC NULLS LAST;

  -- Cursor to select partition definitions
  CURSOR cur_part_defs(obj_id_in IN oph$obj.id%TYPE, type_id_in IN oph_types.t_partition_type_id, partition#_in IN oph_types.t_partition_number DEFAULT NULL) IS
    SELECT k.type_id,
           k.tech_id,
           (SELECT t.name FROM oph$tech t WHERE t.id = k.tech_id) AS partition_technique,
           d.obj_id,
           d.partition#,
           d.subpartition#,
           decode(o.type_id, oph_constants.c_obj_type_table_id, dtc.data_type, oph_constants.c_obj_type_index_id, dicc.data_type) AS data_type,
           --
           ...
           decode(REPLACE(d.high_value, oph_constants.c_apostroph),
                  ...
                  oph_constants.c_maxvalue,
                  oph_constants.c_maxvalue,
                  oph_constants.c_default,
                  oph_constants.c_default,
                  decode(dtc.data_type,
                         oph_constants.c_dt_varchar,
                         oph_constants.c_apostroph || d.high_value || oph_constants.c_apostroph,
                         oph_constants.c_dt_date,
                         oph_constants.c_apostroph || d.high_value || oph_constants.c_apostroph,
                         oph_constants.c_dt_timestamp_0,
                         oph_constants.c_apostroph || d.high_value || oph_constants.c_apostroph,
                         oph_constants.c_dt_timestamp_0_wltz,
                         oph_constants.c_apostroph || d.high_value || oph_constants.c_apostroph,
                         oph_constants.c_dt_timestamp_0_wtz,
                         oph_constants.c_apostroph || d.high_value || oph_constants.c_apostroph,
                         oph_constants.c_dt_timestamp_6,
                         oph_constants.c_apostroph || d.high_value || oph_constants.c_apostroph,
                         oph_constants.c_dt_timestamp_6_wltz,
                         oph_constants.c_apostroph || d.high_value || oph_constants.c_apostroph,
                         oph_constants.c_dt_timestamp_6_wtz,
                         oph_constants.c_apostroph || d.high_value || oph_constants.c_apostroph,
                         oph_constants.c_dt_timestamp_9,
                         oph_constants.c_apostroph || d.high_value || oph_constants.c_apostroph,
                         oph_constants.c_dt_timestamp_9_wltz,
                         oph_constants.c_apostroph || d.high_value || oph_constants.c_apostroph,
                         oph_constants.c_dt_timestamp_9_wtz,
                         oph_constants.c_apostroph || d.high_value || oph_constants.c_apostroph,
           ...
           decode(dtc.data_type,
                  oph_constants.c_dt_date,
                  oph_constants.c_to_date,
                  oph_constants.c_dt_timestamp_0,
                  oph_constants.c_to_timestamp,
                  oph_constants.c_dt_timestamp_0_wltz,
                  oph_constants.c_to_timestamp_tz,
                  oph_constants.c_dt_timestamp_0_wtz,
                  oph_constants.c_to_timestamp_tz,
                  oph_constants.c_dt_timestamp_6,
                  oph_constants.c_to_timestamp,
           ...
           nvl2(d.high_value_format, oph_constants.c_apostroph || d.high_value_format || oph_constants.c_apostroph, NULL) AS high_value_format, -- Set the is_timestamp flag to simplify handling inside the code if data type is DATE or TIMESTAMP
    ...
    -- Join DBA_IND_COLUMNS to get the data type of the partition key for indexes.
     ...
     WHERE k.obj_id = obj_id_in
       AND k.type_id = type_id_in
       AND d.enabled = oph_constants.c_enabled
     ...
     ORDER BY k.tech_id, d.partition#, d.subpartition#;

  /*****************************************************************************
  * Load object definitions
  *****************************************************************************/
  PROCEDURE load_object_defs(obj_defs_out OUT NOCOPY oph_types.t_object_table);

  /*****************************************************************************
  * Handle object
  *****************************************************************************/
  PROCEDURE handle_object(obj_def_in IN oph_objects%ROWTYPE);
...
END oph;
/
CREATE OR REPLACE PACKAGE BODY oph IS
  /*****************************************************************************
  * Constants
  *****************************************************************************/
  -- Default parallel degree
  c_default_parallel_degree CONSTANT BINARY_INTEGER := 2;
  -- Messages
  ...
  ******************************************************************************/
  PROCEDURE add_partition(obj_def_in IN oph_objects%ROWTYPE, part_def_io IN OUT NOCOPY oph_types.t_partition_def) IS
  ...
  BEGIN
    ...
    IF part_def_io.partition_type_id = oph_constants.c_par_type_subpartition_id THEN
    ...
    IF -- RANGE
     part_def_io.partition_tech_id = oph_constants.c_par_tech_range_id THEN
    ...
    -- ORA-14633: Klausel zur Verwaltung von Index bei ADD von List-Unterpartition zu einer zusammengesetzten partitionierten Tabelle nicht zulÃ¤ssig
    IF (part_def_io.partition_type_id = oph_constants.c_par_type_partition_id AND part_def_io.partition_tech_id = oph_constants.c_par_tech_hash_id)
    -- OR (part_def_io.partition_type_id = oph_constants.c_par_type_subpartition_id AND part_def_io.partition_tech_id != oph_constants.c_par_tech_list_id)
     THEN
      v_sql := v_sql || c_update_global_indexes;
    END IF;
  ...
  /* Drop partition
  /******************************************************************************/
  PROCEDURE drop_partition(obj_def_in IN oph_objects%ROWTYPE, part_in IN oph_types.t_partition_info) IS
  ...
  BEGIN
    log_admin.debug(obj_def_in.label_name || ': Try to drop ' || part_in.partition_type || ' ' || CASE WHEN part_in.subpartition_name IS NULL THEN
                    ...
                    module_action_in => v_module_action);
    -- Create SQL to execute
    v_sql := c_alter || obj_def_in.label_name || ' DROP ' || part_in.partition_type || ' ' || CASE
               WHEN part_in.subpartition_name IS NULL THEN
                part_in.partition_name
               ELSE
                part_in.subpartition_name
             END || c_update_global_indexes;
    -- Execute SQL
    sql_admin.execute_sql(sql_in => v_sql);
    -- Log information
  ...
  /******************************************************************************/
  PROCEDURE split_partition(obj_def_in    IN oph_objects%ROWTYPE,
                            split_part_in IN oph_types.t_partition_info,
                            part_def_io   IN OUT NOCOPY oph_types.t_partition_def) IS
  ...
  BEGIN
    ...
    v_sql := c_alter || obj_def_in.label_name || c_split || part_def_io.partition_type || ' ' || CASE
             -- Differentiate by partition type. PARTITION or SUBPARTITION
               WHEN part_def_io.partition_type_id = oph_constants.c_par_type_partition_id THEN
             ...
             CASE part_def_io.partition_tech_id -- Partition by List
               WHEN oph_constants.c_par_tech_list_id THEN
                ' VALUES ' -- Partition by Range
               WHEN oph_constants.c_par_tech_range_id THEN
                ' AT '
             ...
             part_def_io.partition_type || ' ' || CASE
               ...
               ELSE
                split_part_in.subpartition_name
             END || ')' || -- If object type is Table add UPDATE GLOBAL_ INDEXES
             CASE
               WHEN obj_def_in.object_type = oph_constants.c_obj_type_table THEN
  ...
  *****************************************************************************/
  PROCEDURE merge_partitions(obj_def_in               IN oph_objects%ROWTYPE,
                             partition_type_in        IN oph_types.t_partition_type_name,
  ...
  BEGIN
    ...
    v_sql := c_alter || obj_def_in.label_name || c_merge || partition_type_in || 'S ' || first_partition_name_in || ', ' || second_partition_name_in ||
             ' INTO ' || partition_type_in || ' ' || merge_partition_name_in
            -- Add tablespace of the target partition
             || c_tablespace || tablespace_name_in
  ...
  ******************************************************************************/
  PROCEDURE rename_partition(obj_def_in            IN oph_objects%ROWTYPE,
                             part_def_io           IN OUT NOCOPY oph_types.t_partition_def,
  ...
  * Move a partition
  ******************************************************************************/
  PROCEDURE move_partition(obj_def_in IN oph_objects%ROWTYPE, part_def_io IN OUT NOCOPY oph_types.t_partition_def) IS
  ...
  BEGIN
    ...
    v_sql := c_alter || obj_def_in.label_name || CASE
             ...
             END || part_def_io.partition_type || ' ' || part_def_io.partition_name || c_tablespace || part_def_io.tablespace_name || CASE
               WHEN obj_def_in.object_type = oph_constants.c_obj_type_index THEN
                ' ONLINE '
             END || c_parallel || g_parallel_degree;
    -- Execute SQL
    sql_admin.execute_sql(sql_in => v_sql);
  ...
  /* Load object definitions
  /******************************************************************************/
  PROCEDURE load_object_defs(obj_defs_out OUT NOCOPY oph_types.t_object_table) IS
  ...
  /* Load definitions
  /******************************************************************************/
  PROCEDURE load_partition_defs(obj_def_in    IN oph_objects%ROWTYPE,
                                partition#_in IN oph_types.t_partition_number DEFAULT NULL,
                                defs_out      OUT NOCOPY oph_types.t_partition_def_table) IS
  ...
  BEGIN
    IF partition#_in IS NULL THEN
      ...
      n_partition_type_id := oph_constants.c_par_type_partition_id;
    ELSE
      v_def_type          := 'subpartition';
      n_partition_type_id := oph_constants.c_par_type_subpartition_id;
    END IF;
    ...
    OPEN cur_part_defs(obj_id_in => obj_def_in.object_id, type_id_in => n_partition_type_id, partition#_in => partition#_in);
  ...
  /******************************************************************************/
  PROCEDURE load_dba_tab_partitions(obj_def_in IN oph_objects%ROWTYPE, part_info_out OUT NOCOPY oph_types.t_partition_info_table) IS
  ...
  *****************************************************************************/
  PROCEDURE load_dba_tab_subpartitions(obj_def_in    IN oph_objects%ROWTYPE,
                                       part_name_in  IN dba_tab_subpartitions.partition_name%TYPE,
                                       part_info_out OUT NOCOPY oph_types.t_partition_info_table) IS
  ...
  * Load the current existing partition information from data dictionary
  *****************************************************************************/
  PROCEDURE load_dba_ind_partitions(obj_def_in IN oph_objects%ROWTYPE, part_info_out OUT NOCOPY oph_types.t_partition_info_table) IS
  ...
  * Load the current existing subpartition information from data dictionary
  *****************************************************************************/
  PROCEDURE load_dba_ind_subpartitions(obj_def_in    IN oph_objects%ROWTYPE,
                                       part_name_in  IN dba_tab_subpartitions.partition_name%TYPE,
                                       part_info_out OUT NOCOPY oph_types.t_partition_info_table) IS
  ...
  ****************************************************************************/
  PROCEDURE set_last_run_start(object_id_in IN oph$obj.id%TYPE, last_run_start_ts_in IN oph$obj.last_run_start_ts%TYPE DEFAULT systimestamp) IS
  BEGIN
    UPDATE oph$obj o SET o.last_run_start_ts = last_run_start_ts_in WHERE o.id = object_id_in;
    COMMIT;
  ...
  *****************************************************************************/
  PROCEDURE set_last_run_end(object_id_in IN oph$obj.id%TYPE, last_run_end_ts_in IN oph$obj.last_run_end_ts%TYPE DEFAULT systimestamp) IS
  BEGIN
    UPDATE oph$obj o SET o.last_run_end_ts = last_run_end_ts_in WHERE o.id = object_id_in;
    COMMIT;
  ...
  *****************************************************************************/
  FUNCTION get_part_def_convert_literal(part_def_in IN oph_types.t_partition_def, idx_in IN NUMBER) RETURN VARCHAR2 IS
  BEGIN
  ...
  *****************************************************************************/
  PROCEDURE calculate_part_defs_number(part_def_in IN oph_types.t_partition_def, part_defs_io IN OUT NOCOPY oph_types.t_partition_def_table) IS
  ...
  BEGIN
    ...
    FOR i IN -part_def_in.auto_adjust_start .. part_def_in.auto_adjust_end LOOP
      ...
      ELSE
        ...
        oph_tools.convert_literal_to_varchar2(literal_in => get_part_def_convert_literal(part_def_in => part_def_in, idx_in => i),
  ...
  *****************************************************************************/
  PROCEDURE calculate_part_defs_number_mw(part_def_io IN OUT NOCOPY oph_types.t_partition_def) IS
    v_module_action log_admin.t_module_action := 'calculate_part_defs_number_mw';
  BEGIN
    -- Get the current value
    oph_tools.convert_literal_to_number(literal_in => part_def_io.moving_window_function, n_out => part_def_io.high_value);
    -- Save the current high value
    oph_config.update_high_value(obj_id_in        => part_def_io.p_obj_id,
  ...
  *****************************************************************************/
  PROCEDURE calculate_part_defs_varchar(part_def_in IN oph_types.t_partition_def, part_defs_io IN OUT NOCOPY oph_types.t_partition_def_table) IS
  ...
  BEGIN
    ...
    FOR i IN -part_def_in.auto_adjust_start .. part_def_in.auto_adjust_end LOOP
      ...
      part_defs_io(n).high_value := oph_constants.c_apostroph || part_defs_io(n).high_value || oph_constants.c_apostroph;
    END LOOP;
  EXCEPTION
    WHEN e_invalid_identifier THEN
      log_admin.error(SQLERRM || chr(13) || dbms_utility.format_error_backtrace(),
  ...
  *****************************************************************************/
  PROCEDURE calculate_part_defs_ts(part_def_in IN oph_types.t_partition_def, part_defs_io IN OUT NOCOPY oph_types.t_partition_def_table) IS
    v_module_action log_admin.t_module_action := 'calculate_part_defs_ts';
    n_last_idx      PLS_INTEGER := nvl(part_defs_io.last, 0);
    n               PLS_INTEGER := 0;
    --l_v2_tab oph_tools.t_varchar2_tab;
    l_in_tab  oph_types.t_literal_format_tab;
    l_out_tab oph_types.t_ts_wltz_high_value_tab;
  BEGIN
    ...
    -- Convert
    oph_tools.convert_lf_tab_to_ts_hv_tab(lf_tab_in => l_in_tab, ts_hv_tab_out => l_out_tab);
    ...
    FOR i IN l_out_tab.first .. l_out_tab.last LOOP
      ...
      part_defs_io(n_last_idx + i).high_value := oph_constants.c_apostroph || l_out_tab(i).high_value || oph_constants.c_apostroph;
    END LOOP;
  EXCEPTION
    WHEN e_invalid_identifier THEN
      log_admin.error(SQLERRM || chr(13) || dbms_utility.format_error_backtrace(),
  ...
  *****************************************************************************/
  PROCEDURE calculate_part_defs_ts_mw(part_def_in IN oph_types.t_partition_def, part_defs_io IN OUT NOCOPY oph_types.t_partition_def_table) IS
  ...
  BEGIN
    ...
    IF n_current_idx = part_defs_io.first THEN
      ...
      WHILE (NOT b_done) OR (n_def_idx >= n_current_idx - part_def_in.auto_adjust_start) LOOP
        ...
        oph_tools.convert_literal_to_ts_tz(literal_in => get_part_def_convert_literal(part_def_in => part_def_in, idx_in => -i),
                                           ts_tz_out  => part_defs_io(n_def_idx).high_value_ts_tz);
        -- Calculate literal of timestamp high value
        oph_tools.convert_ts_tz_to_literal(ts_tz_in    => part_defs_io(n_def_idx).high_value_ts_tz,
    ...
    IF n_current_idx IS NOT NULL THEN
      oph_config.update_high_value(obj_id_in        => part_defs_io(n_current_idx).p_obj_id,
                                   partition#_in    => part_defs_io(n_current_idx).partition#,
                                   subpartition#_in => part_defs_io(n_current_idx).subpartition#,
                                   high_value_in    => REPLACE(part_defs_io(n_current_idx).high_value, oph_constants.c_apostroph));
      COMMIT;
    END IF;
  EXCEPTION
    WHEN e_invalid_identifier THEN
  ...
  *****************************************************************************/
  PROCEDURE calculate_part_defs(part_def_io IN OUT NOCOPY oph_types.t_partition_def, part_defs_io IN OUT NOCOPY oph_types.t_partition_def_table) IS
    v_module_action log_admin.t_module_action := 'calculate_part_defs';
    v_start_tmp     oph_types.t_auto_adjust_start := part_def_io.auto_adjust_start;
    v_end_tmp       oph_types.t_auto_adjust_end := part_def_io.auto_adjust_end;
  BEGIN
    ...
    oph_tools.convert_literal_to_number(literal_in => v_start_tmp, n_out => part_def_io.auto_adjust_start);
    -- Generate value of auto_adjust_end
    oph_tools.convert_literal_to_number(literal_in => v_end_tmp, n_out => part_def_io.auto_adjust_end);
    -- Case by data type
    IF -- NUMBER
     part_def_io.data_type = oph_constants.c_dt_number THEN
    ...
    ELSIF -- VARCHAR2
     part_def_io.data_type = oph_constants.c_dt_varchar THEN
  ...
  /****************************************************************************/
  PROCEDURE convert_obj_parts_high_value(obj_parts_io IN OUT NOCOPY oph_types.t_partition_info_table) IS
    v_module_action log_admin.t_module_action := 'convert_obj_parts_high_value';
    l_v2_tab        oph_types.t_varchar2_tab;
    l_ts_tab        oph_types.t_timestamp_tab;
  BEGIN
    ...
    FOR i IN obj_parts_io.first .. obj_parts_io.last LOOP
      EXIT WHEN obj_parts_io(i).high_value IN(oph_constants.c_maxvalue, oph_constants.c_default);
    ...
    oph_tools.convert_v2_tab_to_ts_tab(v2_tab_in => l_v2_tab, ts_tab_out => l_ts_tab);
  ...
  /* Convert the high values from existing partitions to real timestamps.
  /****************************************************************************/
  PROCEDURE convert_obj_defs_high_value(obj_defs_io IN OUT NOCOPY oph_types.t_partition_def_table) IS
  ...
  BEGIN
    ...
    FOR i IN obj_defs_io.first .. obj_defs_io.last LOOP
      EXIT WHEN obj_defs_io(i).high_value IN(oph_constants.c_maxvalue, oph_constants.c_default);
  ...
  ******************************************************************************/
  PROCEDURE handle_partition_name(part_def_io IN OUT NOCOPY oph_types.t_partition_def) IS
    ...
    IF part_def_io.partition_naming_function IS NOT NULL THEN
      IF -- NUMBER
       part_def_io.data_type = oph_constants.c_dt_number THEN
        ...
        IF part_def_io.partition_tech_id = oph_constants.c_par_tech_range_id THEN
          -- ... substrate the smallest possible number from the HIGH VALUE to get a number smaller than the HIGH VALUE
          oph_tools.convert_literal_to_varchar2(literal_in => part_def_io.partition_naming_function || '(' ||
        ...
        ELSE
          oph_tools.convert_literal_to_varchar2(literal_in => part_def_io.partition_naming_function || '(' || part_def_io.high_value || ', ''' ||
                                                              part_def_io.partition_name_format || ''')',
                                                v2_out     => part_def_io.partition_name);
        END IF;
      ELSIF -- VARCHAR2
      ...
      ELSIF -- DATE/TIMESTAMP
       part_def_io.is_timestamp = 1 THEN
        ...
        IF part_def_io.partition_tech_id = oph_constants.c_par_tech_list_id THEN
          oph_tools.convert_ts_tz_to_literal(ts_tz_in    => part_def_io.high_value_ts_tz,
        ...
        ELSIF -- If partition definition is RANGE substrate 1s to get a value inside the range
         part_def_io.partition_tech_id = oph_constants.c_par_tech_range_id THEN
          oph_tools.convert_ts_tz_to_literal(ts_tz_in    => part_def_io.high_value_ts_tz - 1 / 86400,
                                             format_in   => part_def_io.partition_name_format,
                                             literal_out => part_def_io.partition_name);
        END IF; -- p_tech_id
      ELSE
  ...
  ******************************************************************************/
  PROCEDURE handle_tablespace_name(part_def_io IN OUT NOCOPY oph_types.t_partition_def) IS
    ...
    IF part_def_io.tablespace_naming_function IS NOT NULL THEN
      ...
      IF -- NUMBER
       part_def_io.data_type = oph_constants.c_dt_number THEN
        IF part_def_io.partition_tech_id = oph_constants.c_par_tech_range_id THEN
          -- ... substrate the smallest possible number from the HIGH VALUE to get a number smaller than the HIGH VALUE
          oph_tools.convert_literal_to_varchar2(literal_in => part_def_io.tablespace_naming_function || '(' ||
                                                              (part_def_io.high_value - to_number(1 / power(10, part_def_io.data_scale))) || ', ''' ||
                                                              part_def_io.tablespace_name_format || ''')',
                                                v2_out     => part_def_io.tablespace_name);
        ELSE
          oph_tools.convert_literal_to_varchar2(literal_in => part_def_io.tablespace_naming_function || '(' || part_def_io.high_value || ', ''' ||
                                                              part_def_io.tablespace_name_format || ''')',
                                                v2_out     => part_def_io.tablespace_name);
        END IF;
      ELSIF -- VARCHAR2
      ...
      ELSIF -- DATE/TIMESTAMP
       part_def_io.is_timestamp = 1 THEN
        IF -- If partition technique is RANGE substrate 1s to get a timestamp inside the range
         part_def_io.partition_tech_id = oph_constants.c_par_tech_range_id THEN
          oph_tools.convert_ts_tz_to_literal(ts_tz_in    => part_def_io.high_value_ts_tz - 1 / 86400,
                                             format_in   => part_def_io.tablespace_name_format,
                                             literal_out => part_def_io.tablespace_name);
        ELSIF -- If partition technique is LIST use the high value
         part_def_io.partition_tech_id = oph_constants.c_par_tech_list_id THEN
          oph_tools.convert_ts_tz_to_literal(ts_tz_in    => part_def_io.high_value_ts_tz,
                                             format_in   => part_def_io.tablespace_name_format,
                                             literal_out => part_def_io.tablespace_name);
        ELSE
  ...
  *****************************************************************************/
  PROCEDURE handle_range_partition(part_def_io IN OUT NOCOPY oph_types.t_partition_def, parts_io IN OUT NOCOPY oph_types.t_partition_info_table) IS
  ...
  BEGIN
    ...
    FOR i IN parts_io.first .. parts_io.last LOOP
      ...
      IF part_def_io.data_type IN (oph_constants.c_dt_number, oph_constants.c_dt_varchar) THEN
        -- First check if the exiting partition is the MAXVALUE partition.
        IF parts_io(i).high_value = oph_constants.c_maxvalue THEN
        ...
        ELSE
          ...
          IF (part_def_io.data_type = oph_constants.c_dt_varchar AND part_def_io.high_value < parts_io(i).high_value) OR
             (part_def_io.data_type = oph_constants.c_dt_number AND
             (part_def_io.high_value <> oph_constants.c_maxvalue AND to_number(part_def_io.high_value) < to_number(parts_io(i).high_value))) THEN
            part_def_io.process_action := c_pa_must_split;
            part_def_io.process_index  := i;
            EXIT;
          END IF;
      ...
      ELSIF part_def_io.is_timestamp = 1 THEN
        ...
        ELSE
          ...
          IF part_def_io.high_value != oph_constants.c_maxvalue THEN
            ...
            IF parts_io(i).high_value_ts_tz IS NULL THEN
              ...
              oph_tools.convert_literal_to_ts_tz(literal_in => parts_io(i).high_value, ts_tz_out => parts_io(i).high_value_ts_tz);
            END IF;
  ...
  ****************************************************************************/
  PROCEDURE handle_list_partition(part_def_io IN OUT NOCOPY oph_types.t_partition_def, parts_io IN OUT NOCOPY oph_types.t_partition_info_table) IS
    v_module_action         log_admin.t_module_action := 'handle_list_partition';
    l_table_part_high_value oph_types.long_varchar;
    l_part_def_high_value   oph_types.mid_varchar;
    l_v2_tab                oph_types.t_varchar2_tab;
  BEGIN
    ...
    ELSE
      ...
      FOR i IN parts_io.first .. parts_io.last LOOP
        ...
        IF -- NUMBER and VARCHAR2
         part_def_io.data_type IN (oph_constants.c_dt_number, oph_constants.c_dt_varchar) THEN
          ...
          oph_tools.convert_v2_to_v2_tab(v2_in => l_table_part_high_value, v2_tab => l_v2_tab);
        ...
        ELSIF --DATE/TIMESTAMP
         part_def_io.is_timestamp = 1 THEN
          ...
          IF (part_def_io.high_value = oph_constants.c_default) AND (parts_io(i).high_value = oph_constants.c_default) THEN
            part_def_io.process_action := c_pa_found;
            part_def_io.process_index  := i;
          ELSIF part_def_io.high_value_ts_tz = parts_io(i).high_value_ts_tz THEN
            part_def_io.process_action := c_pa_found;
    ...
    IF part_def_io.process_action = c_pa_not_found THEN
      ...
      FOR i IN parts_io.first .. parts_io.last LOOP
        IF parts_io(i).high_value = oph_constants.c_default THEN
          part_def_io.process_action := c_pa_must_split;
          part_def_io.process_index  := i;
        END IF;
      END LOOP;
  ...
  *****************************************************************************/
  PROCEDURE handle_partition_definition(obj_def_in  IN oph_objects%ROWTYPE,
                                        part_def_io IN OUT NOCOPY oph_types.t_partition_def,
                                        parts_io    IN OUT NOCOPY oph_types.t_partition_info_table) IS
    v_module_action VARCHAR2(30) := 'handle_partition_definition';
  BEGIN
    ...
    CASE part_def_io.partition_tech_id
      ...
      WHEN oph_constants.c_par_tech_hash_id THEN
        -- Handle Hash partition
        RAISE e_feature_not_implemented;
      WHEN oph_constants.c_par_tech_interval_id THEN
    ...
    CASE part_def_io.process_action
      WHEN c_pa_found THEN
        ...
        IF part_def_io.process_action = c_pa_found THEN
          ...
          IF (part_def_io.partition_type_id = oph_constants.c_par_type_partition_id) AND
          ...
          ELSIF (part_def_io.partition_type_id = oph_constants.c_par_type_subpartition_id) AND
  ...
  *****************************************************************************/
  PROCEDURE handle_object_partitions(obj_def_in IN oph_objects%ROWTYPE, part_defs_io IN OUT NOCOPY oph_types.t_partition_def_table) IS
    v_module_action  log_admin.t_module_action := 'handle_table_partitions';
    l_existing_parts oph_types.t_partition_info_table;
    ...
    FOR i IN part_defs_io.first .. part_defs_io.last LOOP
      IF (i = n_last_idx) OR (part_defs_io(n_last_idx).partition_type <> part_defs_io(i).partition_type) OR
        ...
        IF part_defs_io(i).subpartition# = 0 THEN
          IF obj_def_in.object_type = oph_constants.c_obj_type_table THEN
            load_dba_tab_partitions(obj_def_in => obj_def_in, part_info_out => l_existing_parts);
          ELSE
            load_dba_ind_partitions(obj_def_in => obj_def_in, part_info_out => l_existing_parts);
          END IF;
        ELSE
          -- Load existing subpartitions if the definition is a subpartition (subpartition# > 0)
          IF obj_def_in.object_type = oph_constants.c_obj_type_table THEN
            load_dba_tab_subpartitions(obj_def_in => obj_def_in, part_name_in => part_defs_io(i).parent_partition_name, part_info_out => l_existing_parts);
          ELSE
            load_dba_ind_subpartitions(obj_def_in => obj_def_in, part_name_in => part_defs_io(i).parent_partition_name, part_info_out => l_existing_parts);
          END IF;
        END IF;
      
        -- If partition key is a date or timestamp convert the high value of the existing partitions to a timestamps
        IF part_defs_io(i).is_timestamp = 1 THEN
          convert_obj_parts_high_value(obj_parts_io => l_existing_parts);
        END IF;
      END IF;
  ...
  *******************************************************************************/
  FUNCTION partition_definition_exist(part_in IN oph_types.t_partition_info, part_defs_in IN oph_types.t_partition_def_table) RETURN BOOLEAN IS
  ...
  BEGIN
    ...
    FOR i IN part_defs_in.first .. part_defs_in.last LOOP
      ...
      IF part_defs_in(i).partition_type = oph_constants.c_par_type_subpartition AND part_defs_in(i).parent_partition_name <> part_in.partition_name THEN
        continue;
      END IF;
      -- Check if the definition high value does match the partition high value
      IF part_defs_in(i).data_type IN (oph_constants.c_dt_number, oph_constants.c_dt_varchar) THEN
      ...
      ELSIF part_defs_in(i).is_timestamp = 1 THEN
        ...
        b_result := (part_defs_in(i).high_value_ts_tz = part_in.high_value_ts_tz) OR part_in.high_value IN (oph_constants.c_maxvalue, oph_constants.c_default);
      ELSE
        RAISE e_data_type_not_supported;
      END IF;
    END LOOP;
  ...
  *****************************************************************************/
  PROCEDURE handle_existing_range(obj_def_in       IN oph_objects%ROWTYPE,
                                  part_in          IN oph_types.t_partition_info,
                                  parts_in         IN oph_types.t_partition_info_table,
                                  moving_window_in IN oph_types.t_moving_window) IS
    ...
    IF b_lower_def_found THEN
      ...
      FOR i IN parts_in.first .. parts_in.last LOOP
        ...
        -- independent of the datatype check if there is a MAXVALUE definition to merge in
         (parts_in(i).high_value = oph_constants.c_maxvalue) THEN
          n_higher_def_idx := i;
          EXIT;
        END IF;
      END LOOP;
      ...
      IF n_higher_def_idx != 0 THEN
        IF part_in.subpartition_name IS NULL THEN
          merge_partitions(obj_def_in               => obj_def_in,
                           partition_type_in        => oph_constants.c_par_type_partition,
        ...
        ELSE
          merge_partitions(obj_def_in               => obj_def_in,
                           partition_type_in        => oph_constants.c_par_type_subpartition,
  ...
  * NOT SUPPORTED
  *****************************************************************************/
  PROCEDURE handle_existing_partition(obj_def_in       IN oph_objects%ROWTYPE,
                                      part_in          IN oph_types.t_partition_info,
                                      parts_in         IN oph_types.t_partition_info_table,
                                      moving_window_in IN oph_types.t_moving_window) IS
  ...
  BEGIN
    ...
    IF -- Range Partitioning
     (part_in.subpartition_name IS NULL AND obj_def_in.partition_technique = oph_constants.c_par_tech_range) OR
     (part_in.subpartition_name IS NOT NULL AND obj_def_in.subpartition_technique = oph_constants.c_par_tech_range) THEN
    ...
    ELSIF -- List Partitioning
     (part_in.subpartition_name IS NULL AND obj_def_in.partition_technique = oph_constants.c_par_tech_list) OR
     (part_in.subpartition_name IS NOT NULL AND obj_def_in.subpartition_technique = oph_constants.c_par_tech_list) THEN
    ...
    ELSIF -- Hash Partitioning
     (part_in.subpartition_name IS NULL AND obj_def_in.partition_technique = oph_constants.c_par_tech_hash) OR
     (part_in.subpartition_name IS NOT NULL AND obj_def_in.subpartition_technique = oph_constants.c_par_tech_hash) THEN
      log_admin.notice(obj_def_in.label_name || ' ' || part_in.label_name || ': currently not implemented for LIST partitioning',
                       module_name_in => $$PLSQL_UNIT,
                       module_action_in => v_module_action);
    END IF;
  ...
  *****************************************************************************/
  FUNCTION get_max_moving_window_main(obj_def_in IN oph_objects%ROWTYPE) RETURN oph_types.t_moving_window IS
    n_result oph_types.t_moving_window;
  BEGIN
    SELECT MAX(d.moving_window)
      INTO n_result
      FROM oph$def d
     WHERE d.obj_id = obj_def_in.object_id
  ...
  *****************************************************************************/
  FUNCTION get_max_moving_window_sub(obj_def_in        IN oph_objects%ROWTYPE,
                                     part_defs_in      IN oph_types.t_partition_def_table,
                                     main_part_name_in IN oph_types.t_partition_name) RETURN oph_types.t_moving_window IS
    n_partition# oph_types.t_partition_number := 0;
    n_result     oph_types.t_moving_window;
  BEGIN
    FOR i IN part_defs_in.first .. part_defs_in.last LOOP
      -- Continue if partition type is not PARTITION
      IF part_defs_in(i).partition_type != oph_constants.c_par_type_partition THEN
        continue;
      END IF;
  ...
  /*****************************************************************************
  * Handle existing partitions and drop or merge partitions which are no longer
  * required. First partitions will be handled, then subpartitions.
  *
  * TODO: Check part_defs.count = 0
  *****************************************************************************/
  PROCEDURE handle_existing_partitions(obj_def_in IN oph_objects%ROWTYPE, part_defs_in IN oph_types.t_partition_def_table) IS
    v_module_action log_admin.t_module_action := 'handle_exising_partitions';
    l_parts         oph_types.t_partition_info_table; -- List of existing partitions
    l_subparts      oph_types.t_partition_info_table; -- List of existing subpartitions
    n_main_part_mv  oph_types.t_moving_window;
    n_sub_part_mv   oph_types.t_moving_window;
  BEGIN
    log_admin.debug(obj_def_in.label_name, module_name_in => $$PLSQL_UNIT, module_action_in => v_module_action);
  
    -- Load information about existing partitions
    IF obj_def_in.object_type = oph_constants.c_obj_type_table THEN
      load_dba_tab_partitions(obj_def_in => obj_def_in, part_info_out => l_parts);
    ELSE
      load_dba_ind_partitions(obj_def_in => obj_def_in, part_info_out => l_parts);
    END IF;
    -- The first definition is always a partition definition so this can be used to
    -- see if the high value must be converted if it is a DATE/TIMESTAMP
    IF part_defs_in(1).is_timestamp = 1 THEN
      convert_obj_parts_high_value(obj_parts_io => l_parts);
    END IF;
    ...
    FOR i IN l_parts.first .. l_parts.last LOOP
      -- If no definition exist, handle the partition
      IF NOT partition_definition_exist(part_in => l_parts(i), part_defs_in => part_defs_in) THEN
        handle_existing_partition(obj_def_in => obj_def_in, part_in => l_parts(i), parts_in => l_parts, moving_window_in => n_main_part_mv);
      END IF;
      -- If no subpartitions exist continue
      IF l_parts(i).subpartition_count = 0 THEN
        continue;
      END IF;
      -- Load existing subpartitions
      IF obj_def_in.object_type = oph_constants.c_obj_type_table THEN
        load_dba_tab_subpartitions(obj_def_in => obj_def_in, part_name_in => l_parts(i).partition_name, part_info_out => l_subparts);
      ELSE
        load_dba_ind_subpartitions(obj_def_in => obj_def_in, part_name_in => l_parts(i).partition_name, part_info_out => l_subparts);
      END IF;
      -- If subpartitions are defined, than the second definition is a subpartition. That's why (2) can be used here
      IF part_defs_in(2).partition_type = oph_constants.c_par_type_subpartition AND part_defs_in(2).is_timestamp = 1 THEN
        convert_obj_parts_high_value(obj_parts_io => l_subparts);
      END IF;
      -- Get the maximum value of moving window to know what to do with existing partitions
      -- which are not defined.
      ...
      FOR j IN l_subparts.first .. l_subparts.last LOOP
        IF NOT partition_definition_exist(part_in => l_subparts(j), part_defs_in => part_defs_in) THEN
          handle_existing_partition(obj_def_in => obj_def_in, part_in => l_subparts(j), parts_in => l_subparts, moving_window_in => n_sub_part_mv);
        END IF;
      END LOOP;
    END LOOP;
  EXCEPTION
  ...
  *****************************************************************************/
  PROCEDURE prepare_partition_definitions(obj_def_in IN oph_objects%ROWTYPE, part_defs_out OUT NOCOPY oph_types.t_partition_def_table) IS
    v_module_action log_admin.t_module_action := 'prepare_partition_definitions';
    l_part_defs     oph_types.t_partition_def_table;
    l_tmp_part_defs oph_types.t_partition_def_table;
    l_sub_part_defs oph_types.t_partition_def_table;
    l_tmp_defs      oph_types.t_partition_def_table;
  BEGIN
    log_admin.debug(obj_def_in.label_name, module_name_in => $$PLSQL_UNIT, module_action_in => v_module_action);
    ...
    IF obj_def_in.partition_technique = oph_constants.c_par_tech_reference THEN
      RETURN;
    END IF;
    ...
    FOR i IN l_part_defs.first .. l_part_defs.last LOOP
      ...
      ELSE
        IF -- If the partition definition is a timestamp and it's not the DEFAULT/MAXVALUE partition then convert the high value to timestamp
         (l_part_defs(i).is_timestamp = 1) AND (l_part_defs(i).high_value NOT IN (oph_constants.c_default, oph_constants.c_maxvalue)) THEN
          oph_tools.convert_literal_to_ts_tz(literal_in => get_part_def_convert_literal(part_def_in => l_part_defs(i), idx_in => 0),
    ...
    FOR i IN l_tmp_part_defs.first .. l_tmp_part_defs.last LOOP
      ...
      FOR j IN l_sub_part_defs.first .. l_sub_part_defs.last LOOP
        ...
        ELSE
          ...
          IF (l_sub_part_defs(j).is_timestamp = 1) AND (l_sub_part_defs(j).high_value NOT IN (oph_constants.c_default, oph_constants.c_maxvalue)) THEN
            oph_tools.convert_literal_to_ts_tz(literal_in => get_part_def_convert_literal(part_def_in => l_sub_part_defs(j), idx_in => 0),
  ...
  *******************************************************************************/
  FUNCTION object_is_well_partitioned(obj_def_in IN oph_objects%ROWTYPE) RETURN BOOLEAN IS
    b_result               BOOLEAN;
  ...
  BEGIN
    ...
    ELSIF obj_def_in.object_type = oph_constants.c_obj_type_index THEN
  ...
  *******************************************************************************/
  PROCEDURE redefine_object(obj_def_in IN oph_objects%ROWTYPE, part_defs_in IN oph_types.t_partition_def_table) IS
    v_module_action log_admin.t_module_action := 'redefine_object';
  BEGIN
    oph_redefine.redefine_object(obj_def_in => obj_def_in, part_defs_in => part_defs_in);
  EXCEPTION
    WHEN OTHERS THEN
  ...
  *******************************************************************************/
  PROCEDURE handle_object(obj_def_in IN oph_objects%ROWTYPE) IS
  ...
  BEGIN
    ...
    -- Check if the partition key column does exist
    IF (obj_def_in.object_type = oph_constants.c_obj_type_table AND
       object_admin.is_table_column(owner_in => obj_def_in.object_owner, table_name_in => obj_def_in.object_name, column_name_in => obj_def_in.partition_key) = 0) OR
       (obj_def_in.object_type = oph_constants.c_obj_type_index AND
       object_admin.is_index_column(owner_in => obj_def_in.object_owner, index_name_in => obj_def_in.object_name, column_name_in => obj_def_in.partition_key) = 0) THEN
      RAISE e_invalid_column_name;
    END IF;
  
    -- Check if the subpartition key column does exist if required
    IF (obj_def_in.subpartition_key IS NOT NULL) AND
       ((obj_def_in.object_type = oph_constants.c_obj_type_table AND
       object_admin.is_table_column(owner_in       => obj_def_in.object_owner,
                                      table_name_in  => obj_def_in.object_name,
                                      column_name_in => obj_def_in.subpartition_key) = 0) OR
       (obj_def_in.object_type = oph_constants.c_obj_type_index AND
       object_admin.is_index_column(owner_in       => obj_def_in.object_owner,
                                      index_name_in  => obj_def_in.object_name,
                                      column_name_in => obj_def_in.subpartition_key) = 0)) THEN
      RAISE e_invalid_column_name;
    END IF;
    ...
    IF NOT object_is_well_partitioned(obj_def_in => obj_def_in) THEN
      ...
      IF obj_def_in.object_type = oph_constants.c_obj_type_index THEN
    ...
    ELSE
      ...
      IF obj_def_in.partition_technique != oph_constants.c_par_tech_reference THEN
        ...
        object_admin.rebuild_unusable_indexes(table_owner_in => obj_def_in.object_owner, table_name_in => obj_def_in.object_name);
      END IF;
    END IF;
  ...
  EXCEPTION
    ...
    WHEN e_invalid_column_name THEN
      log_admin.error(c_config_check_required || obj_def_in.label_name || ' partition key column does not exist.',
                      module_name_in => $$PLSQL_UNIT,
                      module_action_in => v_module_action);
    WHEN e_invalid_configuration THEN
  ...
  ******************************************************************************/
  PROCEDURE handle_partitions IS
    ...
    l_obj_defs      oph_types.t_object_table;
  BEGIN
    ...
    -- Check if an object definition was found to avoid ORA-06502
    IF l_obj_defs.count > 0 THEN
      -- Loop through all object definitions and handle them
      FOR i IN l_obj_defs.first .. l_obj_defs.last LOOP
  ...
  ******************************************************************************/
  ...
  BEGIN
    ...
    -- Set the default PARALLEL degree
    ...
    -- Get the database NLS_LANGUAGE
    SELECT VALUE INTO g_nls_lanugage FROM nls_database_parameters WHERE parameter = 'NLS_LANGUAGE';
    -- Set NLS_LANGUAGE to the database language to make sure that automatic generated partition
    -- names will use a consitent language
    sql_admin.execute_sql(sql_in => 'ALTER SESSION SET NLS_DATE_LANGUAGE = ' || g_nls_lanugage);
    -- Enable parallel query and dml to speed up the process
    sql_admin.execute_sql(sql_in => 'ALTER SESSION SET DDL_LOCK_TIMEOUT = 300');
    --sql_admin.execute_sql(sql_in => 'ALTER SESSION FORCE PARALLEL DML PARALLEL 2');
    --sql_admin.execute_sql(sql_in => 'ALTER SESSION FORCE PARALLEL QUERY PARALLEL 2');
