CREATE OR REPLACE PACKAGE oph_redefine AUTHID CURRENT_USER IS
  /*******************************************************************************
  ...
  ******************************************************************************/
  gc_module_version CONSTANT module_admin.t_module_version := '0.9.17';
  gc_module_label   CONSTANT module_admin.t_module_label := $$PLSQL_UNIT || ' v' || gc_module_version;

  PROCEDURE redefine_object(obj_def_in IN oph_objects%ROWTYPE, part_defs_in IN oph_types.t_partition_def_table);
END oph_redefine;
/
CREATE OR REPLACE PACKAGE BODY oph_redefine IS
  /*******************************************************************************
  * Constants
  *******************************************************************************/
  ...
  ******************************************************************************/
  ...
  CURSOR c_table_indexes_ddl(owner_in IN dba_indexes.owner%TYPE, table_name_in IN dba_indexes.table_name%TYPE) IS
    SELECT di.owner,
           ...
           di.uniqueness,
           (SELECT listagg(dic.column_name, ',') within GROUP(ORDER BY dic.column_position)
  ...
  ******************************************************************************/
  ...
  /******************************************************************************/
  FUNCTION get_object_ddl(obj_def_in IN oph_objects%ROWTYPE, part_defs_in IN oph_types.t_partition_def_table) RETURN CLOB IS
    v_module_action          log_admin.t_module_action := 'get_object_ddl';
    v_ref_cons_name          dba_constraints.constraint_name%TYPE;
    n_last_partition_type_id PLS_INTEGER := oph_constants.c_par_type_partition_id;
    l_sql                    CLOB;
  BEGIN
    ...
    IF --REFERENCE partitioning
     obj_def_in.partition_technique = oph_constants.c_par_tech_reference THEN
    ...
    ELSE
      ...
      FOR i IN part_defs_in.first .. part_defs_in.last LOOP
        ...
        IF i != part_defs_in.first THEN
          IF (n_last_partition_type_id = oph_constants.c_par_type_partition_id) AND
             (part_defs_in(i).partition_type_id = oph_constants.c_par_type_subpartition_id) THEN
            dbms_lob.append(dest_lob => l_sql, src_lob => chr(13) || '(');
          ELSIF (n_last_partition_type_id = oph_constants.c_par_type_partition_id) AND
                (part_defs_in(i).partition_type_id = oph_constants.c_par_type_partition_id) THEN
            dbms_lob.append(dest_lob => l_sql, src_lob => ',' || chr(13));
          ELSIF (n_last_partition_type_id = oph_constants.c_par_type_subpartition_id) AND
                (part_defs_in(i).partition_type_id = oph_constants.c_par_type_partition_id) THEN
            dbms_lob.append(dest_lob => l_sql, src_lob => '),' || chr(13));
          ELSIF (n_last_partition_type_id = oph_constants.c_par_type_subpartition_id) AND
                (part_defs_in(i).partition_type_id = oph_constants.c_par_type_subpartition_id) THEN
        ...
        IF part_defs_in(i).partition_tech_id = oph_constants.c_par_tech_range_id THEN
      ...
      IF part_defs_in(part_defs_in.last).partition_type_id = oph_constants.c_par_type_subpartition_id THEN
        dbms_lob.append(dest_lob => l_sql, src_lob => ')');
      END IF;
  ...
  ******************************************************************************/
  ...
  /******************************************************************************/
  PROCEDURE rename_constraints(obj_def_in IN oph_objects%ROWTYPE, constraints_in IN t_table_constraints) IS
  ...
  ******************************************************************************/
  PROCEDURE create_indexes_on_int_table(obj_def_in        IN oph_objects%ROWTYPE,
                                        int_table_name_in IN oph_types.short_varchar,
                                        index_tab_in      IN OUT NOCOPY t_table_index_ddls) IS
    v_module_action oph_types.short_varchar := 'create_indexes';
  BEGIN
