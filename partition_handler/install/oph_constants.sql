CREATE OR REPLACE PACKAGE oph_constants IS
  /****************************************************************************
  * Partition Constants
  *
  * Author:    Thomas Krahn
  ...
  ****************************************************************************/
  ...
  c_obj_type_table    CONSTANT oph$obj_type.name%TYPE := 'TABLE';
  c_obj_type_index_id CONSTANT PLS_INTEGER := 2;
  c_obj_type_index    CONSTANT oph$obj_type.name%TYPE := 'INDEX';
  -- Partition levels
  c_par_type_partition_id    CONSTANT PLS_INTEGER := 1;
  c_par_type_partition       CONSTANT oph$key_type.name%TYPE := 'PARTITION';
  c_par_type_subpartition_id CONSTANT PLS_INTEGER := 2;
  c_par_type_subpartition    CONSTANT oph$key_type.name%TYPE := 'SUBPARTITION';
  -- Partition techniques
  c_par_tech_list_id      CONSTANT PLS_INTEGER := 1;
  c_par_tech_list         CONSTANT oph$tech.name%TYPE := 'LIST';
  c_par_tech_range_id     CONSTANT PLS_INTEGER := 2;
  c_par_tech_range        CONSTANT oph$tech.name%TYPE := 'RANGE';
  c_par_tech_hash_id      CONSTANT PLS_INTEGER := 3;
  c_par_tech_hash         CONSTANT oph$tech.name%TYPE := 'HASH';
  c_par_tech_interval_id  CONSTANT PLS_INTEGER := 4;
  c_par_tech_interval     CONSTANT oph$tech.name%TYPE := 'INTERVAL';
  c_par_tech_reference_id CONSTANT PLS_INTEGER := 5;
  c_par_tech_reference    CONSTANT oph$tech.name%TYPE := 'REFERENCE';
  -- Data types
